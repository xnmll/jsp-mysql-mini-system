...
public class CardDao {
    ...
    public static JsonObject addonetwo(String CardNumber,String CardPassword ,int Point,Timestamp TimeOutDate,Timestamp addTime){
        return JsonUtils.getResult("insert into onetwo (CardNumber,CardPassword,Point,TimeOutDate,addTime) values (?,?,?,?,?)",5,CardNumber,CardPassword,Point,TimeOutDate,addTime);
    }
    //one two
    ...
    //one three
    public static JsonObject addonethree(String CardNumber,String CardPassword ,int Point,Timestamp TimeOutDate,Timestamp addTime){
        return JsonUtils.getResult("insert into onethree (CardNumber,CardPassword,Point,TimeOutDate,addTime) values (?,?,?,?,?)",5,CardNumber,CardPassword,Point,TimeOutDate,addTime);
    }
    //one three
    public static JsonObject deleteonethree(String ids){
        return JsonUtils.getResult("delete from onethree where CardID in(" + ids + ")",0);
    }
    //one three
    public static JsonObject updateonethree(int CardID,String CardNumber,String CardPassword ,int point,String TimeOutDate){
        return JsonUtils.getResult("UPDATE onethree SET CardNumber=?,CardPassword=?,point=?,TimeOutDate=? WHERE CardID=?",5,CardNumber,CardPassword,point,TimeOutDate,CardID);
    }
